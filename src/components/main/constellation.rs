...
impl Clone for FrameTree {
    fn clone(&self) -> FrameTree {
        let mut children = do self.children.iter().transform |child_frame_tree| {
...
impl FrameTree {
    ...
    fn replace_child(&mut self,
        ...
        for self.children.mut_iter().advance |&ChildFrameTree { frame_tree: ref mut child, _ }| {
...
impl Constellation {
    ...
    fn handle_frame_rect_msg(&mut self, pipeline_id: PipelineId, subpage_id: SubpageId, rect: Rect<f32>) {
        ...
        let source_frame = self.current_frame().get().find_mut(pipeline_id);
        for source_frame.iter().advance |source_frame| {
            for source_frame.children.mut_iter().advance |child_frame_tree| {
        ...
        for frame_trees.iter().advance |frame_tree| {
            for frame_tree.children.mut_iter().advance |child_frame_tree| {
                let pipeline = &child_frame_tree.frame_tree.pipeline;
                if pipeline.subpage_id.expect("Constellation: child frame does not have a
                    subpage id. This should not be possible.") == subpage_id {
                    child_frame_tree.rect = Some(rect.clone());
    ...
    fn handle_resized_window_msg(&mut self, new_size: Size2D<uint>) {
        ...
        for self.current_frame().iter().advance |&@FrameTree { pipeline: ref pipeline, _ }| {
            let Size2D { width, height } = new_size;
            pipeline.script_chan.send(SendEventMsg(pipeline.id.clone(),
                                                   ResizeEvent(width, height)));
            already_seen.insert(pipeline.id.clone());
        }
        for self.navigation_context.previous.iter()
            .chain_(self.navigation_context.next.iter())
            .advance |&@FrameTree { pipeline: ref pipeline, _ }| {
